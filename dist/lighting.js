...
import {collections,collectionById,artworkFor,artworkById,artworkOptions,availableCollection,assetPath,selectionKey,selectionLabel} from './artworks.js';
...
let selection={collection:'original',style:'original',time:'day',state:31};
let displayed={...selection},failedSelection=null,drawVersion=0,galleryVersion=0;
let galleryUrls=[];
function parseHash(){
 const hash=location.hash.slice(1);
 if(/^[01]{5}$/.test(hash))return {collection:'original',style:'original',time:'day',state:rooms.reduce((s,r,i)=>s+(hash[i]==='1'?r.bit:0),0)};
 const params=new URLSearchParams(hash),collection=params.get('collection')||'original',style=params.get('style'),time=params.get('time'),lights=params.get('lights');
 if(!artworkFor({collection,style,time})||!/^[01]{5}$/.test(lights||''))return null;
 const artworkId=params.get('artwork')||undefined;
 if(artworkId&&!artworkOptions({collection,style,time}).some(a=>a.id===artworkId))return null;
 return {collection,style,time,artworkId,state:rooms.reduce((s,r,i)=>s+(lights[i]==='1'?r.bit:0),0)};
}
...
function getRenderer(view){
 ...
 const promise=createRenderer(artworkFor(view)).catch(error=>{if(rendererCache.get(key)===promise)rendererCache.delete(key);throw error;});
...
async function getImageUrl(view){
 if(view.state===31){await getRenderer(view);return assetPath(view);}
...
function commitArtwork(view,url){
 const artwork=artworkFor(view);image.src=url;image.width=artwork.width;image.height=artwork.height;
 image.dataset.state=String(view.state);image.dataset.style=view.style;image.dataset.time=view.time;image.dataset.collection=view.collection;image.dataset.artwork=artworkFor(view).id;
 image.alt=`${selectionLabel(view)}. ${stateName(view.state)}.`;
 $('home-art').style.aspectRatio=`${artwork.width}/${artwork.height}`;
 $('room-hit-areas').setAttribute('viewBox',`0 0 ${artwork.width} ${artwork.height}`);
 for(const room of rooms){const region=artwork.regions[room.id];paths.get(room.id).setAttribute('d',region.polygons.map(polygonPath).join(''));labels.get(room.id).style.left=`${100*region.label[0]/artwork.width}%`;labels.get(room.id).style.top=`${100*region.label[1]/artwork.height}%`;}
}
function syncControls(){
 const {state,style,time,collection}=selection,count=rooms.filter(r=>state&r.bit).length;
 ...
 for(const item of collections){const button=$(`collection-${item.id}`);button.setAttribute('aria-pressed',String(item.id===collection));button.disabled=!availableCollection(item.id,selection);}
 ...
 const options=artworkOptions(selection),picker=$('artwork-version');picker.replaceChildren(...options.map(a=>Object.assign(document.createElement('option'),{value:a.id,textContent:a.variantLabel||'Current version'})));picker.value=selectionKey(selection);$('artwork-version-label').hidden=options.length<2;
 const params=new URLSearchParams({collection,style,time,lights:stateKey(state)});if(selection.artworkId)params.set('artwork',selectionKey(selection));history.replaceState(null,'',`#${params}`);
...
async function choose(update){
 ...
 if(!artworkFor(next)||!Number.isInteger(next.state)||next.state<0||next.state>31)throw new Error('Choose a valid style, time, and lighting combination.');
 ...
 try{
  ...
  commitArtwork(next,url);
  displayed={...next};syncFavorite();failedSelection=null;$('lighting-error').hidden=true;
  return true;
 }catch{
...
for(const item of collections)$(`collection-${item.id}`).onclick=()=>choose({collection:item.id});
$('artwork-version').onchange=event=>choose({artworkId:event.target.value});
...
function exportName(state,view){const artwork=artworkFor(view);return stateFile(state,view.style,view.time,view.collection).replace('.png',`${artwork.variant?'-'+artwork.variant:''}.png`);}
function download(blob,name){const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=name;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);}
$('download-current').onclick=async()=>{const view={...displayed};try{const renderer=await getRenderer(view);download(await pngBlob(renderer.render(view.state)),exportName(view.state,view));}catch{reportError('Image could not be saved. Please try again.');}};
...
$('browse-states').onclick=async()=>{
 ...
 try{
  ...
  for(let value=31;value>=0;value--){
   ...
   card.innerHTML=`<img width="320" height="${Math.round(320*renderer.height/renderer.width)}" alt="${stateName(value)}"><span><b>${value===0?'All lights off':value===31?'All lights on':`${rooms.filter(r=>value&r.bit).length} lights on`}</b>${value===0?(view.time==='day'?'Only the daylight.':'Only the night ambience.'):stateName(value)}</span>`;
...
$('download-all').onclick=async()=>{
 const button=$('download-all'),view={...displayed};button.disabled=true;
 try{
  ...
  download(zipFiles(files),`${view.collection}-${view.style}-${view.time}${artwork.variant?'-'+artwork.variant:''}-all-32.zip`);$('announcement').textContent=`All 32 ${selectionLabel(view)} images are ready to download.`;
...
let favoriteVersion=0,favoriteUrls=[],favoriteObserver=null;
function syncFavorite(){
 const saved=isFavorite(selectionKey(displayed),displayed.state);
 $('favorite-view').setAttribute('aria-pressed',String(saved));
 $('favorite-view').textContent=saved?'★ Favorited':'☆ Favorite this view';
 $('favorite-count').textContent=String(favoriteItems().length);
 $('favorites-note').textContent=favoritesPersist()?'Your favorites are saved in this browser.':'Favorites will last for this visit; browser storage is unavailable.';
}
function clearFavoritesGallery(){favoriteVersion++;favoriteObserver?.disconnect();favoriteUrls.forEach(url=>URL.revokeObjectURL(url));favoriteUrls=[];$('favorites-grid').replaceChildren();}
function showFavorites(){
 clearFavoritesGallery();const version=favoriteVersion,items=favoriteItems();
 syncFavorite();if(!$('favorites-dialog').open)$('favorites-dialog').showModal();
 if(!items.length){const empty=document.createElement('p');empty.className='favorites-empty';empty.textContent='Save a view with the star, then find it here.';$('favorites-grid').append(empty);return;}
 let queue=Promise.resolve();
 favoriteObserver=new IntersectionObserver(entries=>{for(const entry of entries){if(!entry.isIntersecting)continue;favoriteObserver.unobserve(entry.target);
  queue=queue.then(async()=>{if(version!==favoriteVersion)return;const card=entry.target,artwork=artworkById(card.dataset.artwork),view={collection:artwork.collection,style:artwork.style,time:artwork.time,artworkId:artwork.id,state:Number(card.dataset.state)};
   try{const renderer=await getRenderer(view),blob=await pngBlob(renderer.render(view.state,320));if(version!==favoriteVersion)return;const url=URL.createObjectURL(blob);favoriteUrls.push(url);card.querySelector('img').src=url;}catch{card.querySelector('img').alt='Preview unavailable; select to retry.';}
  });
 }},{root:$('favorites-dialog'),rootMargin:'100px'});
 for(const item of items){
  const artwork=artworkById(item.artworkId),view={collection:artwork.collection,style:artwork.style,time:artwork.time,artworkId:artwork.id,state:item.state};
  const card=document.createElement('article');card.className='favorite-card';card.dataset.artwork=artwork.id;card.dataset.state=String(item.state);
  const select=document.createElement('button');select.className='favorite-select';select.setAttribute('aria-label',`Open favorite: ${selectionLabel(view)}. ${stateName(item.state)}`);
  const thumb=document.createElement('img');thumb.width=320;thumb.height=Math.round(320*artwork.height/artwork.width);thumb.alt=stateName(item.state);
  const caption=document.createElement('span'),title=document.createElement('b'),state=document.createElement('span');title.textContent=selectionLabel(view);state.textContent=stateName(item.state);caption.append(title,state);select.append(thumb,caption);
  select.onclick=()=>{choose(view);$('favorites-dialog').close();};
  const remove=document.createElement('button');remove.className='remove-favorite';remove.textContent='Remove favorite';remove.setAttribute('aria-label',`Remove favorite: ${selectionLabel(view)}. ${stateName(item.state)}`);remove.onclick=()=>toggleFavorite(artwork.id,item.state);
  card.append(select,remove);$('favorites-grid').append(card);favoriteObserver.observe(card);
 }
}
$('favorite-view').onclick=()=>toggleFavorite(selectionKey(displayed),displayed.state);
$('show-favorites').onclick=showFavorites;
$('close-favorites').onclick=()=>$('favorites-dialog').close();
$('favorites-dialog').addEventListener('close',clearFavoritesGallery);
onFavoritesChanged(()=>{syncFavorite();if($('favorites-dialog').open)showFavorites();});
syncFavorite();
...
if(context?.registerTool){
 ...
 const tool={name:'set_home_view',title:'Set home style and lights',description:'Choose an artwork style, day or night, and room lights in the home dashboard.',inputSchema:{type:'object',properties:{collection:{type:'string',enum:collections.map(c=>c.id)},style:{type:'string',enum:styles.map(s=>s.id)},time:{type:'string',enum:['day','night']},lights:{type:'object',properties:lightProperties,additionalProperties:false}},additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},async execute(input){
  ...
  const next={...selection};if(['collection','style','time'].some(key=>input[key]!==undefined&&input[key]!==selection[key]))delete next.artworkId;if(input.collection!==undefined)next.collection=input.collection;if(input.style!==undefined)next.style=input.style;if(input.time!==undefined)next.time=input.time;
